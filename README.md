...
本适配器面向喜欢动手的技术开发者，如果你只想要开箱即用的体验，官方微信插件可能更合适。以下是中性对比：

| 维度 | wechat_ilink（本插件） | 官方 weixin |
|------|----------------------|------------|
| **架构** | wechatbot-sdk + 本地微信客户端 | aiohttp + cryptography，直连 iLink API |
| **协议** | 依赖本地微信客户端会话 | 纯 API 调用，无需客户端 |
| **依赖** | wechatbot-sdk | aiohttp, cryptography |
| **安装** | 需运行 install.sh 安装运行时补丁 | Hermes 自带，开箱即用 |
| **升级影响** | 运行时注入，Hermes 升级后自动重新应用 | 原生支持，升级后无额外维护 |
| **功能覆盖** | 文本、图片、文件、Cron 投递 | 完整 iLink 协议能力 |
| **代码量** | ~1150 行 | ~1670 行 |

### 为什么选择 wechat_ilink

- 想理解 Hermes Gateway 的运行时注入机制
- 希望对消息收发有更多本地控制
- 学习 Python `sys.meta_path` 导入钩子的实际应用
...
当前运行时注入架构 **技术上可行且已验证通过**，但需要注意：

- **运行时补丁**：通过 `sitecustomize.py` + `sys.meta_path` 注入，Hermes 升级后自动重新应用，只要 Hermes 的模块结构不变，补丁就能继续工作
- **适配层**：wechatbot-sdk 是第三方库，版本更新可能导致兼容性问题，需关注依赖版本
- **协议限制**：iLink 协议本身由微信控制，会话过期（errcode=-14）需要重新扫码，这是协议层面的限制
